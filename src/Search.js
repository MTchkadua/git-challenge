...
export default function Search(props) {
  ...
  return (
    <div className='searchContainer' style={context.modeClick ? {backgroundColor: "#FEFEFE"} : {backgroundColor: "#1E2A47"}} >
      ...
      <input style={context.modeClick ? {color: "#222731", backgroundColor: "#FEFEFE"} : {color : "#FFFFFF", backgroundColor: "#1E2A47"}} className='searchInput' type="text" onChange={(e) => { props.search(e) } } value={props.value} placeholder={"Search GitHyb Username..."} />
      <p className='noResult'> {props.result} </p>
      <button className='searchButton' onClick={props.click}>search</button></>
    </div>
  )
}
